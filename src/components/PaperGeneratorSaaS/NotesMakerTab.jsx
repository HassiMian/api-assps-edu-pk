...
const normalizeClassValue = (value) => String(value || '')
 .toLowerCase()
 .replace(/^class\s+/i, '')
 .replace(/^grade\s+/i, '')
 .replace(/\s+/g, '')
...
function classLevelsMatch(source, target) {
 if (!source || !target) return true
 const targetValue = normalizeClassValue(target)
 const sourceValues = Array.isArray(source) ? source : String(source).split(/[,\s|/]+/)
 return sourceValues.some(value => normalizeClassValue(value) === targetValue)
}
