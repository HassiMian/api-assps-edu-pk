// LessonPlanTab.jsx â€” Al Siddique Smart School OS
...
const loadPlans = () => { try { return JSON.parse(localStorage.getItem(LP_KEY)) || [] } catch { return [] } }
const storePlans = (plans) => localStorage.setItem(LP_KEY, JSON.stringify(plans))

// â”€â”€ Palette â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const C = {
  card:   'rgba(8,24,43,0.96)',
  gold:   '#C8991A', goldL: '#e8b420',
  silver: '#E2E8F0', muted: '#94A3B8',
...
const BLOOM = [
  { key:'remember',   label:'Remember',   color:'#FF375F', emoji:'Recall', verbs:'List, Define, Recall, Name, Identify, Memorise' },
  { key:'understand', label:'Understand', color:'#FF9F0A', emoji:'Understand', verbs:'Explain, Summarise, Describe, Classify, Interpret' },
  { key:'apply',      label:'Apply',      color:'#30D158', emoji:'Apply', verbs:'Use, Solve, Demonstrate, Calculate, Execute' },
  { key:'analyze',    label:'Analyse',    color:'#0A84FF', emoji:'Analyze', verbs:'Compare, Examine, Differentiate, Break down' },
  { key:'evaluate',   label:'Evaluate',   color:'#BF5AF2', emoji:'Evaluate', verbs:'Judge, Justify, Critique, Assess, Defend' },
  { key:'create',     label:'Create',     color:'#C8991A', emoji:'Create', verbs:'Design, Construct, Compose, Plan, Produce' },
]

const PHASES = [
  { key:'intro',    label:'Introduction / Set Induction',  min:5,  icon:'Intro', color:'#0A84FF',  tip:'Hook students, state objectives, activate prior knowledge' },
  { key:'present',  label:'Presentation / Main Teaching',  min:20, icon:'Teach', color:'#30D158',  tip:'Deliver new content, explain concepts, model & demonstrate' },
  { key:'practice', label:'Guided / Independent Practice', min:10, icon:'Practice', color:'#BF5AF2',  tip:'Students practise with teacher support or independently' },
  { key:'closure',  label:'Closure / Summary',             min:5,  icon:'Close', color:'#C8991A',  tip:'Summarise key points, check understanding, preview next lesson' },
]
...
// â”€â”€ Blank Plan Factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const Inp = ({ style = {}, ...props }) => (
  <input {...props} style={{ width: '100%', background: 'rgba(8,24,43,0.95)', border: `1px solid ${C.border}`, borderRadius: 10, color: C.silver, padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box', ...style }} />
)
...
const Txt = ({ style = {}, ...props }) => (
  <textarea {...props} style={{ width: '100%', background: 'rgba(8,24,43,0.95)', border: `1px solid ${C.border}`, borderRadius: 10, color: C.silver, padding: '10px 14px', fontSize: 14, outline: 'none', resize: 'vertical', boxSizing: 'border-box', minHeight: 80, lineHeight: 1.6, ...style }} />
)
const Btn = ({ children, onClick, variant = 'gold', style: s = {}, disabled = false }) => {
  const v = {
    ...
    ghost:  { background: 'rgba(8,24,43,0.90)', color: C.silver, border: `1px solid ${C.border}` },
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ display:'flex', alignItems:'center', gap:7, padding:'9px 18px', borderRadius:10, fontWeight:700, fontSize:13, cursor: disabled ? 'not-allowed' : 'pointer', whiteSpace:'nowrap', opacity: disabled ? 0.5 : 1, ...v[variant], ...s }}>{children}</button>
  )
}
...
function Toast({ message, color = C.green, onClose }) {
  return (
    ...
    <div className="super-module-card" style={{ position:'fixed', bottom:28, right:28, zIndex:9999, background:'rgba(7,30,52,0.98)', border:`1px solid ${color}55`, borderRadius:14, padding:'14px 20px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 8px 32px rgba(0,0,0,0.5)', maxWidth:360 }}>
      <div className="super-module-card" style={{ width:36, height:36, borderRadius:'50%', background:`${color}20`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:900, flexShrink:0, color }}>OK</div>
      ...
      <button onClick={onClose} style={{ background:'none', border:'none', color:C.muted, cursor:'pointer', fontSize:18, lineHeight:1 }}>Close</button>
    </div>
    </Portal>
  )
}

// â”€â”€ Auto-Generate Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className="super-module-card" style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div className="super-module-card" style={{ background:'#071e34', border:`1px solid ${C.border}`, borderRadius:20, padding:28, width:560, maxHeight:'90vh', overflowY:'auto' }}>
        <div className="super-module-card" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:22 }}>
          ...
          <button onClick={onClose} style={{ background:'rgba(255,55,95,0.12)', border:'1px solid rgba(255,55,95,0.3)', borderRadius:8, color:C.red, cursor:'pointer', padding:'6px 12px', fontWeight:700 }}>Close</button>
        </div>
          ...
          <div className="super-module-card" style={{ marginTop:16 }}>
            ...
            <div className="super-module-card" style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
              <button onClick={() => setSelChapters(selChapters.length === availChapters.length ? [] : [...availChapters])}
                style={{ fontSize:11, padding:'4px 10px', borderRadius:8, border:`1px solid ${C.border}`, background: selChapters.length === availChapters.length ? 'rgba(200,153,26,0.2)' : 'rgba(8,24,43,0.90)', color:C.gold, cursor:'pointer', fontWeight:700 }}>
...
// â”€â”€ Plan List Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className="super-module-card" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, overflow: 'hidden' }}>
      ...
      <div className="super-module-card" style={{ padding: '18px 20px' }}>
        ...
        <div className="super-module-card" style={{ display:'flex', gap:12, fontSize:12, color:C.muted, marginBottom:12, flexWrap:'wrap' }}>
          {plan.teacher && <span>Teacher: {plan.teacher}</span>}
          <span>Date: {plan.date}</span>
          <span>Duration: {plan.duration} min</span>
          <span>Planned: {totalMin} min</span>
        </div>
        {plan.bloom.length > 0 && (
          <div className="super-module-card" style={{ display:'flex', gap:4, flexWrap:'wrap', marginBottom:12 }}>
            {plan.bloom.map(k => {
              const b = BLOOM.find(x => x.key === k)
              return b ? <span key={k} style={{ fontSize:10, padding:'2px 7px', borderRadius:20, background:`${b.color}18`, color:b.color, border:`1px solid ${b.color}44`, fontWeight:700 }}>{b.label}</span> : null
            })}
          </div>
        )}
        <div className="super-module-card" style={{ display:'flex', gap:8, marginTop:4, flexWrap:'wrap' }}>
          <Btn variant="gold" onClick={() => onPreview(plan)} style={{ flex:1, justifyContent:'center' }}>Preview / Print</Btn>
          <Btn variant="purple" onClick={() => onSendPortal(plan)} style={{ justifyContent:'center' }}>Portal</Btn>
          <Btn variant="ghost" onClick={() => onEdit(plan)}>Edit</Btn>
          <Btn variant="red" onClick={() => onDelete(plan.id)}>Delete</Btn>
        </div>
      </div>
    </div>
  )
...
// â”€â”€ Mini Plan Card (for Weekly view) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <div>
      ...
      <div className="super-module-card" style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20, flexWrap:'wrap' }}>
        <button onClick={() => setWeekOffset(p=>p-1)} style={{ background:'rgba(8,24,43,0.90)', border:`1px solid ${C.border}`, borderRadius:10, padding:'8px 16px', color:C.silver, cursor:'pointer', fontWeight:700 }}>Prev</button>
        ...
        <button onClick={() => setWeekOffset(0)} style={{ background: weekOffset===0 ? 'rgba(200,153,26,0.2)':'rgba(8,24,43,0.90)', border:`1px solid ${weekOffset===0?C.gold:C.border}`, borderRadius:10, padding:'8px 14px', color: weekOffset===0?C.gold:C.silver, cursor:'pointer', fontWeight:700, fontSize:12 }}>Today</button>
        <button onClick={() => setWeekOffset(p=>p+1)} style={{ background:'rgba(8,24,43,0.90)', border:`1px solid ${C.border}`, borderRadius:10, padding:'8px 16px', color:C.silver, cursor:'pointer', fontWeight:700 }}>Next</button>
      </div>
...
// â”€â”€ Annual View â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <div>
      <div className="super-module-card" style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
        <button onClick={() => setYear(y=>y-1)} style={{ background:'rgba(8,24,43,0.90)', border:`1px solid ${C.border}`, borderRadius:10, padding:'8px 16px', color:C.silver, cursor:'pointer', fontWeight:700 }}>Previous {year-1}</button>
        <div className="super-module-card" style={{ flex:1, textAlign:'center' }}>
        ...
        <button onClick={() => setYear(y=>y+1)} style={{ background:'rgba(8,24,43,0.90)', border:`1px solid ${C.border}`, borderRadius:10, padding:'8px 16px', color:C.silver, cursor:'pointer', fontWeight:700 }}>Next {year+1}</button>
      </div>
...
// â”€â”€ Plan Editor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      {groups.length === 0 ? (
        ...
        <GCard key={group.title}>
          <div className="super-module-card" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            <button onClick={() => onNew(group.items[0]?.date)} style={{ background:'rgba(200,153,26,0.12)', border:`1px solid ${C.gold}44`, color:C.gold, borderRadius:10, padding:'7px 10px', cursor:'pointer', fontWeight:800 }}>+ Add</button>
          </div>
    ...
    <div>
        ...
        {SECTIONS.map((s, i) => (
          <button key={i} onClick={() => setActiveSection(i)}
            style={{ padding:'8px 16px', borderRadius:10, border:'none', cursor:'pointer', fontWeight:700, fontSize:12, whiteSpace:'nowrap',
              background: activeSection === i ? `linear-gradient(135deg, ${C.gold}, ${C.goldL})` : 'rgba(8,24,43,0.96)',
            ...
            <div className="super-module-card" style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:10 }}>
              {plan.materials.map((m, i) => (
                <span key={i} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 12px', borderRadius:20, background:'rgba(10,132,255,0.12)', border:'1px solid rgba(10,132,255,0.25)', color:C.blue, fontSize:13, fontWeight:600 }}>
                  {m}
                  <button onClick={() => delMaterial(i)} style={{ background:'none', border:'none', color:C.blue, cursor:'pointer', fontSize:16, lineHeight:1, padding:0 }}>Ã—</button>
                </span>
              ))}
            </div>
      ...
      {activeSection === 1 && (
        <div className="super-module-card" style={{ display:'grid', gap:20 }}>
          <GCard>
            <SectionHead icon="Goals" title="Learning Objectives" />
              ...
              {plan.objectives.map((obj, i) => (
                <div key={i} style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
                  ...
                  {plan.objectives.length > 1 && (
                    <button onClick={() => delObjective(i)} style={{ background:'rgba(255,55,95,0.12)', border:'1px solid rgba(255,55,95,0.25)', borderRadius:8, color:C.red, cursor:'pointer', padding:'9px 12px', fontWeight:700 }}>Delete</button>
                  )}
                </div>
              ))}
            </div>
            <button onClick={addObjective} style={{ marginTop:12, background:'rgba(200,153,26,0.1)', border:`1px dashed ${C.gold}`, borderRadius:10, padding:'9px 0', color:C.gold, cursor:'pointer', width:'100%', fontWeight:700, fontSize:13 }}>
              + Add Objective
            </button>
          </GCard>
          <GCard>
            <SectionHead icon="Bloom" title="Bloom's Taxonomy - Cognitive Levels" color={C.purple} />
              ...
              {BLOOM.map(b => {
                ...
                return (
                  <div key={b.key} onClick={() => toggleBloom(b.key)}
                    ...
                    <div className="super-module-card" style={{ fontSize:16, fontWeight:800, marginBottom:6 }}>{b.emoji}</div>
          ...
          <GCard style={{ padding:'16px 24px' }}>
            <div className="super-module-card" style={{ display:'flex', gap:16, alignItems:'center', flexWrap:'wrap' }}>
              ...
              <span style={{ fontSize:13, fontWeight:800, color: timeOk ? C.green : C.orange, marginLeft:'auto' }}>
                {timeOk ? 'OK' : 'Check'} Total: {totalMin} / {plan.duration} min
              </span>
            </div>
          </GCard>
          {plan.phases.map(ph => {
      ...
      {activeSection === 3 && (
        <div className="super-module-card" style={{ display:'grid', gap:20 }}>
          <GCard>
            <SectionHead icon="Assessment" title="Assessment Strategy" color={C.green} />
            <div className="super-module-card" style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:16 }}>
              {ASSESSMENT_TYPES.map(t => (
                <button key={t} onClick={() => set('assessmentType', t)}
                  style={{ padding:'7px 14px', borderRadius:20, border:`1px solid ${plan.assessmentType === t ? C.green : C.border}`, background: plan.assessmentType === t ? 'rgba(48,209,88,0.15)' : 'rgba(8,24,43,0.90)', color: plan.assessmentType === t ? C.green : C.muted, cursor:'pointer', fontWeight:600, fontSize:12 }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <GCard>
            <SectionHead icon="Homework" title="Homework / Assignment" color={C.orange} />
      ...
      {activeSection === 4 && (
        <div className="super-module-card" style={{ display:'grid', gap:20 }}>
          <GCard>
            <SectionHead icon="Support" title="Differentiation" color={C.purple} />
          ...
          <GCard>
            <SectionHead icon="Reflection" title="Post-Lesson Reflection" color={C.blue} />
      ...
      <div className="super-module-card" style={{ display:'flex', justifyContent:'space-between', marginTop:24 }}>
        ...
        <Btn variant="gold" onClick={() => onSave(plan)}>Save Plan</Btn>
        <Btn variant="ghost" onClick={() => setActiveSection(s => Math.min(4, s+1))} style={{ opacity: activeSection===4?0.3:1, pointerEvents: activeSection===4?'none':'auto' }}>Next â†’</Btn>
      </div>
    </div>
  )
}

// â”€â”€ Print Preview â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <div>
      ...
      <div id="lp-print-area" style={{ background:'#fff', color:'#000', padding:'32px 36px', maxWidth:780, margin:'0 auto', borderRadius:8, fontFamily:'Times New Roman, serif', fontSize:13, lineHeight:1.6, boxShadow:'0 8px 40px rgba(0,0,0,0.4)' }}>
        <div className="super-module-card" style={{ textAlign:'center', marginBottom:16, borderBottom:'2px solid #8B6914', paddingBottom:12 }}>
          {settings?.logo && <img src={settings.logo} alt="logo" style={{ height:60, objectFit:'contain', display:'block', margin:'0 auto 8px' }} />}
          {settings?.showUrduHeader !== false && <div className="super-module-card" style={{ fontFamily:'Noto Nastaliq Urdu, serif', fontSize:20, direction:'rtl', marginBottom:4 }}>{settings?.schoolUrdu || 'Al Siddique Scholars Public School'}</div>}
          <div className="super-module-card" style={{ fontSize:20, fontWeight:700 }}>{settings?.schoolName || 'Al Siddique Scholars Public School'}</div>
          <div className="super-module-card" style={{ fontSize:11, color:'#555' }}>{settings?.address || 'Sharif Chowk, Rayya Khas, Narowal'}</div>
          <div className="super-module-card" style={{ fontSize:17, fontWeight:700, marginTop:8, color:'#5a3e00', letterSpacing:2, textTransform:'uppercase' }}>{scopeLabel(plan.planningScope)} Lesson Plan</div>
          <div className="super-module-card" style={{ fontSize:12, color:'#555', marginTop:4 }}>{plan.planRangeLabel || planRangeLabel(plan.planningScope || 'daily', plan.date)}</div>
        </div>

        {studentName.trim() && (
          <div className="super-module-card" style={{ border:'1px solid #bbb', background:'#f8f5ed', padding:'8px 12px', marginBottom:12, fontSize:13 }}>
        ...
        <div className="super-module-card" style={{ display:'flex', justifyContent:'space-between', marginTop:32, borderTop:'1px solid #ccc', paddingTop:20 }}>
          {['Teacher\'s Signature','Head Teacher\'s Signature','Principal\'s Signature'].map(s => (
            <div key={s} style={{ textAlign:'center', width:200 }}>
              <div className="super-module-card" style={{ borderBottom:'1px solid #555', marginBottom:6, height:40 }} />
              <div className="super-module-card" style={{ fontSize:12, color:'#555' }}>{s}</div>
            </div>
          ))}
        </div>

        <div className="super-module-card" style={{ textAlign:'center', fontSize:10, color:'#aaa', marginTop:16 }}>
...
// â”€â”€ Main LessonPlanTab â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export default function LessonPlanTab({ settings }) {
  ...
  return (
    <div>
      ...
      {confirmDel && (
        ...
        <div className="super-module-card" style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div className="super-module-card" style={{ background:'#071e34', border:`1px solid ${C.border}`, borderRadius:18, padding:28, width:340, textAlign:'center' }}>
            <div className="super-module-card" style={{ fontSize:36, marginBottom:12 }}>Delete</div>
      ...
      {/* â”€â”€ LIST / WEEKLY / ANNUAL â”€â”€ */}
      {view === 'list' && (
        <div>
          {/* Header */}
          <GCard style={{ marginBottom:20, display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
            <div className="super-module-card" style={{ fontSize:40 }}>Plans</div>
            <div className="super-module-card" style={{ flex:1 }}>
            ...
            <div className="super-module-card" style={{ display:'flex', gap:4 }}>
              {[['list','List'],['weekly','Weekly'],['annual','Annual']].map(([v,l]) => (
                <button key={v} onClick={() => setPlanView(v)}
                  style={{ padding:'7px 14px', borderRadius:10, border:`1px solid ${planView===v?C.gold:C.border}`, background: planView===v?'rgba(148,163,184,0.18)':'rgba(8,24,43,0.90)', color: planView===v?C.gold:C.muted, fontWeight:700, fontSize:12, cursor:'pointer' }}>
                  {l}
                </button>
              ))}
            </div>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
              style={{ background:'rgba(8,24,43,0.95)', border:`1px solid ${C.border}`, borderRadius:12, color:C.silver, padding:'10px 16px', fontSize:13, outline:'none', width:180 }} />
          ...
          {planView === 'list' && (
            filtered.length === 0 ? (
              <GCard style={{ textAlign:'center', padding:60 }}>
                <div className="super-module-card" style={{ fontSize:52, marginBottom:16 }}>Plans</div>
