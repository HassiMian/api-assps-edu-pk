...
async function main() {
  ...
  for (const tableName of Object.keys(tables)) {
    ...
    if (columns.includes("school_id")) {
      ...
      if (updateSchoolRes.rowCount > 0) {
        console.log(`Locked ${updateSchoolRes.rowCount} rows in [${tableName}] to school_id: ${schoolId}`);
      }
    }
    ...
    if (columns.includes("tenant_id")) {
      ...
      if (updateTenantRes.rowCount > 0) {
        console.log(`Locked ${updateTenantRes.rowCount} rows in [${tableName}] to tenant_id: ${tenantId}`);
      }
    }
  }
