...
const InlineInput = ({ value, onChange, placeholder = '', style = {}, multiline = false }: any) => {
...
export default function BoardPaperGenerator({ loadedPaper, onReturnToSource }: any) {
...
<style>
  *,*::before,*::after{box-sizing:border-box}
  ...
  [data-edit-guide]{border:none!important}
  hr{display:block}
</style>
  ...
  const updateMainQ = (sIdx: number, qIdx: number, fields: any) => {
    const next = [...paper.sections]
  ...
  const updatePart = (sIdx: number, qIdx: number, pIdx: number, fields: any) => {
    const next = [...paper.sections]
    const mq = [...next[sIdx].mainQuestions]
  ...
  const addMainQuestion = (sIdx: number) => {
    const next = [...paper.sections]
    const mq = next[sIdx].mainQuestions || []
  ...
  const addPart = (sIdx: number, qIdx: number) => {
    const next = [...paper.sections]
    const mq = [...next[sIdx].mainQuestions]
    const parts = mq[qIdx].parts || []
        ...
        <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, padding: 20, maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
            <div>
              ...
              <InlineInput value={paper.subject} onChange={(v: string) => setPaper({ ...paper, subject: v })} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 4 }}>Class</div>
              <InlineInput value={paper.classLevel} onChange={(v: string) => setPaper({ ...paper, classLevel: v })} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 4 }}>Total Marks</div>
              <InlineInput value={paper.totalMarks} onChange={(v: string) => setPaper({ ...paper, totalMarks: v })} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 4 }}>Time Allowed</div>
              <InlineInput value={paper.timeAllowed} onChange={(v: string) => setPaper({ ...paper, timeAllowed: v })} />
            </div>
          </div>
          ...
          {paper.sections?.map((sec: any, sIdx: number) => (
            <div key={sIdx} style={{ marginBottom: 24, padding: 12, background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: `1px solid ${C.border}` }}>
              ...
              <div style={{ marginBottom: 6 }}>
                ...
                <InlineInput value={sec.title} onChange={(v: string) => updateSection(sIdx, { title: v })} style={{ fontWeight: 600 }} />
              </div>
              <div style={{ marginBottom: 6 }}>
                <div style={{ fontSize: 10, color: C.muted }}>Instructions</div>
                <InlineInput value={sec.instructions} onChange={(v: string) => updateSection(sIdx, { instructions: v })} multiline />
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: C.muted }}>Marks</div>
                <InlineInput value={sec.marks} onChange={(v: string) => updateSection(sIdx, { marks: v })} />
              </div>

              {/* Main Questions */}
              <div style={{ paddingLeft: 12, borderLeft: `2px solid rgba(255,255,255,0.05)` }}>
                {sec.mainQuestions?.map((mq: any, qIdx: number) => (
                  <div key={qIdx} style={{ marginBottom: 16, padding: 10, background: 'rgba(0,0,0,0.15)', borderRadius: 8 }}>
                     <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                       ...
                       <InlineInput value={mq.qNumber} onChange={(v: string) => updateMainQ(sIdx, qIdx, { qNumber: v })} style={{ fontSize: 11, width: 40 }} />
                       <span style={{ marginLeft: 'auto', color: C.silver, fontSize: 11, fontWeight: 600 }}>Marks:</span>
                       <InlineInput value={mq.marks} onChange={(v: string) => updateMainQ(sIdx, qIdx, { marks: v })} style={{ fontSize: 11, width: 40 }} />
                     </div>
                     <div style={{ marginBottom: 8 }}>
                       <InlineInput value={mq.text} onChange={(v: string) => updateMainQ(sIdx, qIdx, { text: v })} multiline style={{ fontSize: 12 }} />
                     </div>

                     {/* Parts */}
                     {mq.parts?.length > 0 && (
                       <div style={{ display: 'grid', gap: 8, paddingLeft: 10, marginBottom: 8 }}>
                         {mq.parts.map((p: any, pIdx: number) => (
                           <div key={pIdx} style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
                              <InlineInput value={p.partId} onChange={(v: string) => updatePart(sIdx, qIdx, pIdx, { partId: v })} style={{ fontSize: 11, width: 30, color: C.gold }} />
                              <div style={{ flex: 1 }}>
                                <InlineInput value={p.text} onChange={(v: string) => updatePart(sIdx, qIdx, pIdx, { text: v })} multiline style={{ fontSize: 12 }} />
                              </div>
                              <button onClick={() => {
